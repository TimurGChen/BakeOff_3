...
void drawKeyboard() {
  ...
  for (int i=0; i<keyRows.length; i++) {
    ...
    for (int j=0; j<curRow.length; j++) {
      ...
      if (curButton.isOn()) {
        fill(0, 255, 0);
        //rect(curX, curY, keyWidth * 1.5, keyHeight * 1.5);
      } else {
        fill(255);
      ...
      if(currentTyped.length() != 0 && ((currentTyped.length() < currentPhrase.length()))) {
        if(currentPhrase.charAt(currentTyped.length() - 1) == currentTyped.charAt(currentTyped.length() - 1)) {
          ...
          if (currentPhrase.charAt(currentTyped.length()) == ' ') {
            rect(spaceX, spaceY, spaceWidth * 1.5, keyHeight * 1.5);
          }
        } else if (currentTyped.length() != 0){
          rect(delX, delY, delWidth * 1.5, keyHeight * 1.5);
        }
      }
